---
slug: "/2019-07-03-huong-dan-su-dung-react-hook-effect"
date: "2019-07-03"
title: "Sử dụng React Hook Effect"
desc: "Bài này mình contribute cho http://vi.reactjs.org về cách sử dụng React Hook Effect"
cover: ""
type: "post"
lesson: 0
chapter: 0
...
Đoạn snippet này dựa trên [ví dụ về counter ở trang trước](http://vi.reactjs.org/docs/hooks-state.html), chúng ta có thêm tính năng mới: đặt giá trị document title tương ứng với số lần click.
...
>Tip
>
>Không giống `componentDidMount` hoặc `componentDidUpdate`, effect chạy với `useEffect` không block trình duyệt cập nhập màn hình. Các effect chủ yếu không cần xảy ra tuần tự. Trong vài tình huống không mấy phổ biến (ví dụ như đo layout), chúng ta có  [`useLayoutEffect`](http://vi.reactjs.org/docs/hooks-reference.html#uselayouteffect) Hook với API tính năng tương tự như `useEffect`.
...
**Nếu bạn đã nắm bắt được cách làm việc của Effect Hook, hoặc nếu bạn cảm thấy hơi ngợp, có thể nhảy xuống [phần Nguyên tắc sử dụng Hook](http://vi.reactjs.org/docs/hooks-rules.html) bây giờ.**
...
Như vậy hook đã giải quyết vấn đề này như thế nào? Nếu như [bạn có thể sử dụng *State* Hook nhiều lần](http://vi.reactjs.org/docs/hooks-state.html#tip-using-multiple-state-variables), bạn cũng có thể sử dụng nhiều effect. Nó cho phép tách những logic không liên quan ra thành những effect khác nhau:
...
>Lưu ý
>
>Nếu sử dụng cách tối ưu này, phải chắc chắn array chứa **tất cả giá trị bên trong của component scope (như prop và state) nếu thay đổi theo các lần render và effect có sử dụng**. Nếu không, nếu không nó sẽ tham chiếu tới giá trị trước đó. Đọc thêm [làm việc với function](http://vi.reactjs.org/docs/hooks-faq.html#is-it-safe-to-omit-functions-from-the-list-of-dependencies) và [làm gì khi array thay đổi thường xuyên](http://vi.reactjs.org/docs/hooks-faq.html#what-can-i-do-if-my-effect-dependencies-change-too-often).
>
...
>
>Nếu truyền vào array rỗng (`[]`), prop và state bên trong effect sẽ luôn mang giá trị khởi tạo. Trong khi truyền vào `[]` nó gần giống với `componentDidMount` và `componentWillUnmount`, nó thường là [giải pháp](http://vi.reactjs.org/docs/hooks-faq.html#what-can-i-do-if-my-effect-dependencies-change-too-often) [tốt hơn](http://vi.reactjs.org/docs/hooks-faq.html#is-it-safe-to-omit-functions-from-the-list-of-dependencies) để tránh re-run effect quá thường xuyên. Tuy nhiên, đừng quên React sẽ chỉ chạy `useEffect` sau khi trình duyệt paint.
>
...
Xin chúc mừng! Trang này không hề ngắn, nhưng bạn đã đọc được đến đây. Hy vọng các thắc mắc của bạn về effect đã được phúc đáp. Bạn đã học được State Hook và Effect Hook, và có *rất* nhiều thứ bạn có thể làm khi sử dụng kết hợp chúng.  Nó gần như giải quyết các vấn đề mà chỉ có class mới làm được -- còn nếu không bạn có thể tìm thấy [các Hook mở rộng](http://vi.reactjs.org/docs/hooks-reference.html).

Chúng ta đã trình bài động lực tạo ra Hook [ở đây](http://vi.reactjs.org/docs/hooks-intro.html#motivation). Chúng ta cũng thấy được cách  effect cleanup và tránh trùng lặp trong `componentDidUpdate` và `componentWillUnmount`, mang những đoạn code có liên quan lại gần nhau hơn, và giúp chúng ta tránh bug. Chúng ta cũng thấy được cách chúng ta tách effect theo mục đích, cái mà chúng ta không làm được với class.

Đến đây, bạn có thể thắc mắc Hook làm việc như thế nào. Làm cách nào `useState` lấy được đúng giá trị giữa các lần render khác nhau? Làm cách nào React "match" được effect trước và lần đang update? **Ở trang tiếp theo bạn sẽ học được [Quy luật của Hook](http://vi.reactjs.org/docs/hooks-rules.html) -- mấu chốt làm việc của Hook.**
